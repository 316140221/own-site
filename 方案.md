...
- GitHub Actions 定时运行（MVP：每 2 小时，UTC，可按数据源限制调整），自动更新并部署 Pages。
...
- `/feed.xml`：站点 RSS（最新条目）
- `/category/<name>/`：分类聚合
- `/category/<name>/feed.xml`：分类 RSS
- `/languages/`：语言列表
- `/lang/<code>/`：按语言聚合（可分页）
- `/lang/<code>/feed.xml`：语言 RSS
- `/p/<id>/`：条目页
...
- 失败处理：单源失败不影响整体；抓取超时与重试；连续失败源自动退避；在 GitHub Actions Summary 输出失败/退避列表。
...
- 归档+清理（可选）：`ARCHIVE_OLD=1 npm run update`（归档输出到 `archives/`，默认不提交到仓库）
- 本地预览：`npm run dev`（11ty 预览）
- 构建发布：`npm run build:site`（11ty + Pagefind）

### 13.2 GitHub Pages 注意点

- Actions 部署：已提供 workflow（每 2 小时触发一次，UTC）。
- 站点 URL 与路径前缀：
  - `SITE_URL`：用于 `sitemap.xml`/canonical/RSS 绝对链接（建议设置为站点域名，例如 `https://<owner>.github.io`，不要包含 `/<repo>/`）
