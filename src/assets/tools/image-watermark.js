(function () {
  ...
  async function loadImage(file) {
    ...
    const ok = new Promise((resolve, reject) => {
      img.onload = () => resolve();
      img.onerror = () => reject(new Error(t("tool.imageWatermark.error.decode")));
    });
    img.src = url;
    await ok;
    return { img, url };
  ...
  function drawWatermark(ctx, opts) {
    ...
    const fontSize = Math.max(6, Math.round(opts.fontSize || 28));
    const margin = Math.max(0, Math.round(opts.margin || 0));
    const opacity = clampNumber(opts.opacity, 0.05, 1) ?? 0.25;
    const color = opts.color || "#ffffff";
    const outline = opts.outline === true;
    const pos = String(opts.position || "br");
    ...
    if (pos === "tr") {
      ...
      ctx.textAlign = "left";
      x = margin;
      y = Math.max(margin, canvasH - margin - totalHeight);
    } else if (pos === "center") {
      ctx.textAlign = "center";
      x = Math.round(canvasW / 2);
      y = Math.max(margin, Math.round((canvasH - totalHeight) / 2));
    } else {
      ctx.textAlign = "right";
      x = canvasW - margin;
      y = Math.max(margin, canvasH - margin - totalHeight);
    ...
    const lineWidth = Math.max(2, Math.round(fontSize / 10));
  ...
  async function run({ showErrors } = { showErrors: true }) {
    const file = getFile();
    if (!file) {
      if (showErrors) setStatus(t("tool.imageWatermark.error.noFile"), true);
    ...
    if (!String(file.type || "").startsWith("image/")) {
      setStatus(t("tool.imageWatermark.error.noFile"), true);
      return;
    }
    ...
    const colorRaw = String($("opt-color").value || "").trim();
    if (colorRaw && !isValidHexColor(colorRaw)) {
      if (showErrors) setStatus(t("tool.imageWatermark.error.color"), true);
      return;
    }
    const color = colorRaw ? expandHexColor(colorRaw) : "#ffffff";

    setStatus(t("tool.imageWatermark.status.working"), false);
    ...
    $("meta-original").textContent = [file.type || "image", dimsText, formatBytes(file.size)]
      .filter(Boolean)
      .join(" · ");
    ...
    if (!targetW || !targetH) {
      setStatus(t("tool.imageWatermark.error.decode"), true);
      return;
    }
    ...
    if (!ctx) {
      setStatus(t("tool.imageWatermark.error.unsupported"), true);
      return;
    }
    ...
    drawWatermark(ctx, {
      ...
      fontSize: clampNumber($("opt-size").value, 6, 400) ?? 28,
      opacity: clampNumber($("opt-opacity").value, 0.05, 1) ?? 0.25,
      color,
      margin: clampNumber($("opt-margin").value, 0, 500) ?? 18,
      outline: $("opt-outline").checked,
    });
    ...
    const outName = `${base}-watermark.${outExt}`;
    ...
    $("meta-output").textContent = [blob.type || outMime, formatDims(targetW, targetH), formatBytes(blob.size)]
      .filter(Boolean)
      .join(" · ");

    const lines = [];
    lines.push(`Original: ${file.name} · ${file.type || "image"} · ${dimsText} · ${formatBytes(file.size)}`);
    lines.push(`Output: ${outName} · ${blob.type || outMime} · ${formatDims(targetW, targetH)} · ${formatBytes(blob.size)}`);
    $("tool-output").value = lines.join("\n");

    setStatus(t("tool.imageWatermark.status.done"), false);
  }
  ...
  function main() {
    try {
      ...
      fileInput.addEventListener("change", scheduleAutoRun);
